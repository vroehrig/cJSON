...
char *cJSONUtils_FindPointerFromObjectTo(cJSON *object, cJSON *target)
{
    ...
    for (obj = object->child; obj; obj = obj->next, c++)
    {
        ...
        if (found)
        {
            ...
            {
                char *ret = (char*)malloc(strlen(found) + cJSONUtils_PointerEncodedstrlen(obj->name) + 2);
                *ret = '/';
                cJSONUtils_PointerEncodedstrcpy(ret + 1, obj->name);
                strcat(ret, found);
                free(found);
...
cJSON *cJSONUtils_GetPointer(cJSON *object, const char *pointer)
{
    ...
    while ((*pointer++ == '/') && object)
    {
        ...
        else if ((object->type & 0xFF) == cJSON_Object)
        {
            object = object->child;
            /* GetObjectItem. */
            while (object && cJSONUtils_Pstrcmp(object->name, pointer))
            {
                object = object->next;
            }
...
static int cJSONUtils_ApplyPatch(cJSON *object, cJSON *patch)
{
    ...
    {
        /* Get rid of old. */
        cJSON_Delete(cJSONUtils_PatchDetach(object, path->value.string));
        if (opcode == 1)
        {
    ...
    if ((opcode == 3) || (opcode == 4))
    {
        ...
        {
            /* move */
            value = cJSONUtils_PatchDetach(object, from->value.string);
        }
        if (opcode == 4)
        {
            /* copy */
    ...
    parentptr = cJSONUtils_strdup(path->value.string);
        ...
        case cJSON_Object:
        {
            ...
            while (a || b)
            {
                ...
                if (!diff)
                {
                    ...
                    char *newpath = (char*)malloc(strlen(path) + cJSONUtils_PointerEncodedstrlen(a->name) + 2);
                    cJSONUtils_PointerEncodedstrcpy(newpath + sprintf(newpath, "%s/", path), a->name);
                ...
                else if (diff < 0)
                {
                    ...
                    cJSONUtils_GeneratePatch(patches, "remove", path, a->name, 0);
                    a = a->next;
                }
                else
                {
                    /* object element doesn't exist in 'from' --> add it */
                    cJSONUtils_GeneratePatch(patches, "add", path, b->name, b);
                    b = b->next;
                }
            }
            return;
